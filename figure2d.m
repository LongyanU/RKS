...
for it=1:nt-2,
    ...
    p2_prime = p - dt *v.^2.* (dvx_dx + dvy_dy);  %k-1/2

    % Stage 3'
    [dpx_dx, dpy_dy] = spatial_derivatives(p);  %k+1/2
    vx2_prime = Vx + dt * dpx_dx;               %k+1
    vy2_prime = Vz + dt * dpy_dy;               %k+1

    % Stage 4'
    [dvx_dx, dvy_dy] = Vx_Vz_spatial_derivatives(vx2_prime, vy2_prime,coeff);
    p3_prime = p + dt *v.^2.* (dvx_dx + dvy_dy);  %k+3/2
