...
% d= [1.24275, -0.113493, 0.0272555, -0.00731429, 0.00173544, -0.000303651, 0.0000284348];
d = [1.55427, -0.27162, 0.07394, -0.0208847, 0.005117, -0.000919339, 0.0000882575];
...
for ii=1:100
    ...
    for aa=1:5
        ...
        for cdc=0.79:0.0001:1.2
            tempD1=0;
            for n=1:7
                tempD1=tempD1+2*d(n)* sin((n-0.5)*kh(ii)*sin(xita))*2* sin((0.5)*kh(ii)*sin(xita));
            end

            tempD2=0;
            for n=1:7
                tempD2=tempD2+2*d(n)*sin((n-0.5)*kh(ii)*cos(xita))*2*sin((0.5)*kh(ii)*cos(xita));
            end

            D=sqrt(tempD1+tempD2);
...
for ii=1:5
    if ii==1
        hold on;plot(100*kh/(pi),a1(ii,:),'r-.','LineWidth',1)
    elseif ii==2
        hold on;plot(100*kh/(pi),a1(ii,:),'b--','LineWidth',1)
    elseif ii==3
        hold on;plot(100*kh/(pi),a1(ii,:),'c--','LineWidth',1)
